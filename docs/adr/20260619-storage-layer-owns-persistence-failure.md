...
- **Recovery is retry + backup, not a write guarantee.** The failure indicator opens a detail dialog that always offers a full configuration backup (`saveLocalForageToFile`) — it is read-mostly, so it remains viable even under quota pressure, and it lets the user preserve whatever did persist regardless of the failure reason. We do **not** block reload (`beforeunload`).
