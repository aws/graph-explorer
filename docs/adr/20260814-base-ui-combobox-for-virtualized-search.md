...
- **A fully hand-rolled combobox** was built first (see the discussion in PR #2131). It re-implemented accessible combobox semantics from scratch — ARIA roles, keyboard navigation, focus management, positioning — which is exactly the kind of well-tested, easy-to-get-subtly-wrong surface a primitives library exists to own. It was replaced rather than kept.
...
- **`useVirtualizer` needs a lint suppression only.** The call in `Combobox.tsx` carries a `// eslint-disable-next-line react-compiler/incompatible-library` comment to silence a lint rule; the React Compiler still bails out on `useVirtualizer` itself. Deleting the comment fails `pnpm check:lint`. See `docs/agents/react.md`.
- **The trigger/input interaction pattern is VoiceOver-validated, not just ARIA-linted.** The trigger button is kept in the accessibility tree and pointer-operable on purpose, but deliberately not a Tab stop (its `tabIndex` is `-1`): when the input already has a value, VoiceOver's Read-All treats it as content to read and skips announcing its combobox role — the button is what re-announces "combo box" in that case (confirmed against shadcn's Base UI reference implementation, which does the same). An earlier revision hid it from screen readers to avoid VoiceOver reading Control-Option-Space on a grouped control as "stop interacting with this group" — if that resurfaces, re-validate it with a screen reader rather than re-deriving from ARIA best practices. The input itself gets an explicit `onClick` handler to open the list, because a text input has no native "click" default action the way a `<button>` does, and VoiceOver's Control-Option-Space gesture needs one to trigger reliably. Base UI's own Trigger is what makes a second click reliably close the list instead of racing the library's own open-state handling.
- **Only visible options mount.** `VirtualizedOptions` renders ~20 DOM nodes regardless of total option count; the virtualizer must measure `Combobox.List` (the bounded, scrollable viewport), not the inner spacer div that is deliberately as tall as the full list — measuring the wrong element defeats virtualization silently (it "works" but renders nearly everything).
- **Imports and keys are deliberate.** Base UI is imported via the `@base-ui/react/combobox` subpath to keep the dependency scoped to one file, and virtualized rows use TanStack's index keys (`virtualItem.key`) because Base UI already indexes items as `<inputId>-<index>` and the code does not use `measureElement`; switching to value keys would fight the library.
