...
A link resolves to one of four intents against the existing connections: `none` (it targets the active connection, so do nothing), `activate` (it matches an inactive connection, so switch to it), `create` (no match, so open a pre-filled form), or `invalid` (a param failed validation, so warn naming each bad param and ignore the link). A param fails validation when `queryEngine` or `serviceType` is unsupported, `awsRegion` isn't shaped like a region, or `graphDbUrl` is missing, malformed, carries credentials, or contains a backslash. The `#/connect` route exists only for connection links, so a missing `graphDbUrl` is one more invalid-link case rather than a distinct "no link" outcome; there is no `none`-by-default path into this route. A connection matches only when its `graphDbUrl` (normalized the same way a stored connection's is), `queryEngine`, **and auth posture** all agree, where auth posture is IAM on/off and, when on, the region and service type.
