...
A reusable 1.19M-triple RDF fixture lives in the named graph `<http://graph-explorer.test/filter-benchmark>` on the development Neptune cluster, with 100,000 `Person` resources across 8 literal attributes, 4,948 subjects carrying repeated predicates, and 20 `Organization` hubs from 1,000 to 50,000 members. Ground-truth counts are recorded for each filter combination. Both candidate shapes were also confirmed to parse on Neptune 1.2.1.0 and 1.3.5.0, so neither introduces a version-floor regression.
