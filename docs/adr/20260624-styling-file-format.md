...
- **Parse is atomic; load is selective.** Parsing is validate-or-nothing: a file with any invalid value parses to nothing and reports every offending location, so the user never lands in a partially-styled state from a malformed file. Once a file parses, the selective import preview opens and the user chooses which types to load — so "not applying every type" is a deliberate per-type choice, distinct from the whole-file parse rejection. The terminal UI states are the selective preview (on a valid file with actionable styles), a "nothing to load" notice (empty or all-already-matching file), an envelope-level failure, or an invalid-values report.
