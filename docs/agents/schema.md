...
node type; it does not fall back to a full-page error. `SchemaGraphToolbar`
renders `EdgeConnectionDiscoveryStatusButton`, a warning icon button next to
"Refresh Schema" that opens a popover with the details on click. It resolves
the situation via the pure `edgeConnectionNotice(schema, error)` helper in
...
synchronizing both call `edgeDiscoveryQuery.refetch()` and close the popover.
