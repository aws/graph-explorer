...
- Prefer **data attributes** for conditional styles. Tailwind v4 provides two forms:
  - `data-open:` / `data-closed:` — shorthand variants (defined via `@custom-variant` in `index.css`) that match `[data-state="open"]` / `[data-state="closed"]`, the Radix convention. Note: this **redefines** Tailwind's native behavior, where a bare `data-open:` would match the presence of a `data-open` attribute. Also: `aria-invalid:` for form validation.
    - **Base UI components don't emit `data-state`.** They set bare boolean attributes (`data-open`, `data-closed`, `data-starting-style`, `data-ending-style`), so the project's `data-open:`/`data-closed:` shorthand — which is redefined in `index.css` to match Radix's `[data-state="open"]`/`[data-state="closed"]` — will silently never match a Base UI element. For `data-open:` and `data-closed:` only, use the arbitrary-value form with bare-attribute matching (`data-[open]:`, `data-[closed]:`). Every other bare `data-*` variant (`data-starting-style:`, `data-ending-style:`, `data-highlighted:`, etc.) matches by attribute presence and can use the native `data-...:` shorthand. See `docs/adr/20260814-base-ui-combobox-for-virtualized-search.md`.
