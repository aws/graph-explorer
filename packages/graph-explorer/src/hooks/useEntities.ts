...
// let's test it using the following code:
// Path: packages/graph-explorer/src/hooks/useEntities.test.ts

describe("useEntities", () => {
  it("should filter nodes and edges", () => {
    // given
    const nodes = [
      {
        data: {
    ...
    const filteredNodesIds = new Set(["1"]);
    const filteredEdgesIds = new Set(["1"]);
    const vertexTypes = new Set(["type1"]);
    const connectionTypes = new Set(["type1"]);
    const disableFilters = false;

    given


    when_the_entity_is_filtered();
    then_the_entity_is_filtered();
  });

// create tests useEntities for a total 6 tests with the most coverage
// 1. when the entity is filtered
// 2. when the entity is not filtered
// 3. when the entity is filtered by global filters
// 4. when the entity is not filtered by global filters
// 5. when the entity is filtered by global filters and disableFilters is true
// 6. when the entity is not filtered by global filters and disableFilters is true
