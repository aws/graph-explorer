...
const entitiesSelector = selector<Entities>({
  ...
  set: ({ get, set }, newEntities) => {
    ...
    const nonUnconnectedEdges = nonDupEdges.filter(edge => {
      return nonDupNodes.has(edge.source) && nonDupNodes.has(edge.target);
    });
