...
describe("resolveServerConfig", () => {
  ...
  describe("NEPTUNE_NOTEBOOK conflicts with an explicit HTTPS request", () => {
    const conflicting = createTestEnvironment({
      NEPTUNE_NOTEBOOK: true,
      PROXY_SERVER_HTTPS_CONNECTION: true,
    });

    it("refuses to start, naming both variables", () => {
      // Certificates present, so a missing-certificate error cannot be what
      // fires here.
      vi.spyOn(fs, "existsSync").mockReturnValue(true);

      let message = "";
      expect(() => resolveServerConfig(conflicting)).toThrow(
        expect.objectContaining({ name: "ServerConfigError" }),
      );
      try {
        resolveServerConfig(conflicting);
      } catch (e) {
    ...
    it("names the conflict rather than the certificates when they are missing", () => {
      vi.spyOn(fs, "existsSync").mockReturnValue(false);
    ...
    it("allows HTTPS on its own when the certificates exist", () => {
      vi.spyOn(fs, "existsSync").mockReturnValue(true);
